G.AddData({
	name:'Thomas',
	author:'Thomas',
	desc:'Legacy with a few small changes.',
	engineVersion:1,
	manifest:0,
	func:function()
	{
		...
		*/
			...
			tick:function(me,tick)
			{
				...
				if (me.amount>0)
				{
					...
					if(G.has('fire-making')) {
						G.gain('happiness',-leftout*0.15,'cold & darkness');
					} else {
		...
		new G.Unit({
			...
			effects:[
				...
				{type:'convert',from:{'clay':2},into:{'clay tablet':1},every:10,mode:'clay tablets'},
				{type:'mult',value:1.2,req:{'ground stone tools':true}}
			],
			req:{'carving':true},
			category:'crafting',
		...
		new G.Tech({
			name:'city planning',
			desc:'@unlocks [architect]s@[grave]s take up less land<>',
			icon:[22,8],
			cost:{'insight':25},
		...
		new G.Achiev({
			...
			fromUnit:'mausoleum',
			effects:[
				{type:'addFastTicksOnStart',amount:3000},
